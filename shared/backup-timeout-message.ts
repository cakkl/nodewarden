...
//   ② 后端按它判断「是不是超时」⇒ 决定回 **400（不可重试）** 还是 500
//      （500 会被前端 `retryableRequest` 自动重试 3 次，把一次超时放大成三倍等待）；
//   ③ 前端按它把消息映射成**本地化文案**（`webapp/src/lib/i18n.ts`，还把毫秒换算成秒）。
//
// 原先 ② 与 ③ 各写了一份正则：任何一侧改措辞，另一侧都会**静默失配** ——
// 用户看到英文原文，或者超时被当成 500 触发自动重试。现在三者共用本文件，
// 测试里的样例也由 `buildRemoteTimeoutMessage()` 生成 ⇒ 改一处就会被测试拦住。
