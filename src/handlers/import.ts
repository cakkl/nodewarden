...
export async function handleCiphersImport(request: Request, env: Env, userId: string): Promise<Response> {
  ...
  //
  // 原来这里先把 folders 批量写库，然后才在循环里逐条校验 ciphers —— 于是一条非法条目会让函数返回
  // 400，而**文件夹已经写进去了**：用户看到"导入失败"，库里却多出几个空文件夹；重试一次多一批（每次
  // 都是新的 UUID，不会冲突，只会累积）。
  //
  // 现在写入统一挪到校验之后：导入要么整体成功，要么什么都不留下。（无法做到真事务：D1 的 batch 只
  // 保证单批原子，跨批次需要显式事务，而 5000 条塞进一个事务并不合适。前移校验足以覆盖"入参非法"这
  // 一唯一失败原因。）
