 ...
 *
 * 必须有它：`isTotpEnabled()` 只看“有没有值”，而 `base32Decode()` 碰到 A–Z2–7 之外的字符会返回
 * null ⇒ 表现为「两步验证显示已启用、但**任何码都不对**」的隐蔽死锁。
 *
 * 分工：**写入 / 启用路径用本函数**（非法密钥在启用那一刻就被拒绝）；**读取路径（登录时判断要不要
 * 收两步验证）继续用 `isTotpEnabled`** 以保持 **fail-closed** —— 库里存了坏密钥时仍要求两步验证，
 * 而不是静默降级成“只要密码就能登录”。坏密钥的出路是恢复码，或在设置页重新启用一次。
 */
