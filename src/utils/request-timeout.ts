 ...
 * 外发请求的超时封装（共用实现）。
 *
 * 为什么必须有：对端「连上了但不回包」时（黑洞 IP、防火墙丢包、挂住的 TLS 握手），`fetch` 可能
 * **永不 settle** ⇒ 异常永远抛不出来 ⇒ 调用方的 catch 永不执行，最后由平台兜底返回通用 500，
 * 运维只拿到一个引用号，无法自助排查。
 *
 * 约定：包的是**整段操作**（发送请求 + 读响应体）。`fetch` 在收到响应头时就 resolve 了，「发了头就
 * 不再发数据」卡住的正是 `await response.text()` / `arrayBuffer()` 那一步 —— 所以 `run` 里要把读
 * body 也一起做完（见 `backup-uploader.ts` 的下载路径）。
 */
