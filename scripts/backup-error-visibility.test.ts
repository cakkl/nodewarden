// 「备份失败 / 被跳过」的可观察性护栏（docs/TODO.md 第 18、19 条）。
//
// 两条都是 2026-09-17 真机验收时发现的**静默**问题 —— 功能没坏，但人看不到发生了什么：
//
//   第 18 条：每次尝试**一开始**就把 `lastErrorMessage` 清空，而失败不更新 `lastSuccessAt`
//             ⇒ 计划任务在容差窗口内立刻重试 ⇒ 「清空 → 30 s 后写回 → 立刻又清空」循环，
//             于是那条错误几乎永远看不到（当时只能靠审计日志确认一直在失败）。
//             修法：清空只发生在**成功**分支。
//   第 19 条：租约被别人持有时 DO 回 409，而 handler 侧**直接 return**
//             ⇒ 「这一轮计划任务被跳过」没有任何留痕（不报错、日志里全 200）。
//             修法：`console.warn` + 一条 `system` 审计事件。
//
// 为什么第 18 条用源码断言而不是跑一遍备份：要观测到「尝试开始时是否清空」，
// 必须在两次尝试之间读 `runtime`，而真实落库发生在 restore/backup 的中途，
// 单测里只能靠 stub 掉整条远端链路 —— 收益低于成本。源码断言更精确：
// 它直接锁住「那段 update 里不许出现 lastError*」，改回去立刻红。
//
