// Yubico 外发请求的超时守卫
//
// 为什么值得测：`src/utils/yubico-otp.ts` 有**两处**外发请求，原先都没有超时。
// 对端（api.yubico.com / upgrade.yubico.com）连上但不回包时：
//   · 登录的二步验证会**挂住**，最后由平台兜底 500 —— 用户既登不进去，也看不到原因；
//   · 管理员启用 YubiKey 时的「取 API 凭据」同样挂住。
// 加了超时之后有两条**不可退化**的语义：
