...
//
// 必须单独测：Send（"分享"）是**未认证用户也能访问**的功能面 —— 其他 handler 都躲在登录后面，
// 只有这里能被任何人用 URL 打到。它同时守三件事：
//   ① **不可泄露**：拿不到 accessId 的人猜不到内容；"不存在"与"无权访问"必须同一种回答
//   ② **一次性语义**：达到 `maxAccessCount` 后必须真的失效，且**并发下也不能超发**
//   ③ **密码爆破要有代价**：错了要计数、要能锁
//
// 另一部分覆盖 `sends-shared.ts` 的**纯函数**（解析、可用性判定、accessId 编解码、JWT 密钥
// 校验）：不需要数据库、用例密度大，而改动风险恰好集中在这些门控上。
//
