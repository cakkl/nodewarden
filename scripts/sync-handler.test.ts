...
//
// 它是客户端每次启动都会走的端点、**全量返回**，客户端拿到后整体替换本地状态 ⇒ 两个方向的风险：响应里
// 少了东西 = 客户端删本地数据；多了非法的东西 = 解析失败、彻底进不去。
//
// 所以重点不在“字段对不对”，而在：
//   · 缓存键隔离 —— 键由 用户/修订号/凭据标签/三个标志位 拼成，任一维度漏进去就会跨请求串答案（先请求
//     `?excludeSends=1` 的响应被缓存后，下一个请求命中它、于是客户端清空本地 Sends）。
//   · 污染行不扩散 —— `account-passkeys.ts` 完全不碰 revisionDate，所以凭据标签是“新增 passkey 后
//     客户端能否立刻看到”的唯一保障；而 `name` 不是合法 EncString 的历史脏数据会被丢掉，而不是让整个
//     同步失败。
//
