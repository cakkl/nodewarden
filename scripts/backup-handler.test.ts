...
//
// 为什么值得测：备份/恢复是**唯一直接关系数据存亡**的功能面，而且是**管理员专用**的 —— 13 个端点
// 全部吃 `actorUser`，任何一个漏掉权限判断都等于把整库导出能力开放给普通用户。
//
//   ① **权限扫描**：13 个管理端点逐个用"非管理员"调一遍，全部必须 403。写成循环而不是抄 13 遍 ——
//      将来新增端点只要加进数组，忘了守卫就会红。
//   ② **本地导出**（含附件）：勾了"包含附件"的 zip 必须**真的带附件字节**，而不是只有一个自称
//      "包含附件"的清单。
//
