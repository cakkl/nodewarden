...
// `/\.+$/`、`/^\/+|\/+$/g`）。这些正则本身是线性扫描，但审计噪音不值得长期挂着，因此全部改成
// **显式循环**。改了就必须证明"语义没变"，否则这类改动最容易悄悄改变规范化结果（域名规范化直接
// 影响域名规则的匹配）。
//
// 本文件做三件事：① 特征化断言：循环实现与"被替换掉的正则链"在一批含恶意长串的输入上结果一致；
// ② 端到端断言：`normalizeEquivalentDomain` 对超长恶意输入是线性的、结果正确；
// ③ 源码护栏：那 4 个文件里不允许再出现尾部量词正则（防止回退）。
