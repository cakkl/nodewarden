...
//
// 为什么值得单独测：附件是**唯一把数据写进 R2 对象存储**的功能面，风险比纯 D1 的 handler 多一层：
//
//   ① **两个存储要一致**：删除必须同时清掉 R2 对象与 D1 元数据。只删一边会产生"数据库说没有、R2 里
//      还躺着用户文件"（隐私问题）或"数据库说有条目、下载 404"（坏体验）。
//   ② **下载授权走签名 URL**：`?token=...` 是唯一凭证，必须绑定到具体 cipher + attachment，且
//      **只能用一次**（`consumeAttachmentDownloadToken`）。
//   ③ **上传是一次性的**：同一附件重复上传应被 409 挡住，避免元数据与实际内容对不上。
//
