// webapp 纯逻辑测试：站点 URL 解析与图标地址
//
// 为什么先测这几个：它们是"点了之后算得对不对"里最基础的一层 —— 从条目里挑出要显示的网站、从任意
// 形状的用户输入里解析出主机名。用户输入极其随意（带不带 scheme、带不带端口、粘了路径或查询串、全角、
// 前后空白），这里的容错能力直接决定"网站图标能不能显示出来"。
//
