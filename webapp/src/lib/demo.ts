...
export const DEMO_AUDIT_LOGS: AuditLogEntry[] = [
  // 说明：以下条目的 action / category / level / targetType / metadata **照着服务端真实写入
  // 的内容写**（对应 src/handlers/*.ts 里的 writeAuditEvent / writeAuditLog 调用），
  // 不是自创的名字。原因是日志中心会把「语言包里没有对应键的动作名」回退成
  // "Auth / Login / Failed" 这种斜杠标题 —— 演示数据一旦和真实动作名不一致，
  // 页面看起来就和真实程序不一样，示例也就失去了意义。
  // 同理，metadata 只用 audit-events.ts 白名单里的键：真实事件里不认识的键会被丢弃。
  {
  ...
  {
    // 真实来源：identity.ts 密码登录成功 —— auth / info / user
    ...
    metadata: JSON.stringify({
      grantType: 'password',
      webSession: true,
      deviceIdentifier: 'demo-device-browser',
      deviceType: 9,
      method: 'POST',
  ...
  {
    // 真实来源：identity.ts 密码错误 —— auth / warn / user。
    // 真实事件不会往 metadata 里写 reason（"密码错"已由动作名表达），所以这里也不写。
  ...
  {
    // 真实来源：accounts.ts 启用两步登录 —— security / security / user
  ...
  {
    // 真实来源：identity.ts 刷新登录失败 —— auth / warn / **refreshToken**，
    // 且真实调用**不写操作者**（刷新失败时还没有通过认证的用户），界面显示 "—"。
    // reason 用真实取值 token_not_found_or_expired（语言包里也有对应译文）。
    id: 'demo-log-auth-refresh-failed',
    actorUserId: null,
    actorEmail: null,
    action: 'auth.refresh.failed.token_not_found_or_expired',
    category: 'auth',
    level: 'warn',
  ...
  {
    // 真实来源：accounts.ts 修改主密码 —— security / security / user，metadata.email
  ...
  {
    // 真实来源：admin.ts 改用户状态。该动作以 admin.user. 开头 ⇒ 助手按规则给
    // security / security（其余 admin.* 动作是 system / info），targetType = user
    ...
    metadata: JSON.stringify({
      status: 'banned',
      method: 'PUT',
      path: '/api/admin/users/demo-user-003',
      ip: '203.0.113.42',
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/125.0.0.0 Safari/537.36',
    }),
  ...
  {
    // 真实来源：accounts.ts 用邀请码注册 —— security / info / user。
    // 注意：真实代码往 metadata 里塞了 inviteCode，但它**不在白名单里**会被丢弃，
    // 所以这里也不写它（否则演示数据会比真实程序"多"一个字段）。
  ...
  {
    // 真实来源：devices.ts 永久信任设备 —— device / security / device，metadata.updated
  ...
  {
    // 真实来源：devices.ts 删除设备 —— device / **security**（不是 warn），metadata.deleted
    ...
    metadata: JSON.stringify({
      deleted: 1,
      method: 'DELETE',
      path: '/api/devices/demo-device-old',
      ip: '198.51.100.20',
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/125.0.0.0 Safari/537.36',
    }),
  ...
  {
    // 真实来源：devices.ts 批量撤销设备信任 —— device / security / **user**，
    // metadata 是 requested / removed 两个计数
  ...
  {
    // 真实来源：backup.ts 定时远端备份成功。
    // 这条**不带** method/path/ip/userAgent：手动备份（`trigger: 'manual'`）会由调用方把
    // auditRequestMetadata(request) 传进来，而定时任务跑在 Durable Object 的闹钟里、
    // 没有 Request 对象，所以那四个字段根本不会出现。
    id: 'demo-log-data-backup',
    actorUserId: DEMO_USER_ID,
    actorEmail: DEMO_PROFILE.email,
    action: 'admin.backup.remote.scheduled',
    category: 'system',
    level: 'info',
    targetType: 'backup',
    targetId: null,
    ...
    metadata: JSON.stringify({
      ...
      destinationId: 'demo-destination-r2',
      destinationType: 'r2',
      provider: 'r2',
      remotePath: 'nodewarden/backups/nodewarden_backup_2026-09-06.zip',
      fileName: 'nodewarden_backup_2026-09-06.zip',
      fileBytes: 1258291,
      uploadVerificationAttempts: 1,
      prunedFileCount: 1,
    }),
  ...
  {
    // 真实来源：backup.ts 从远端恢复（导入）—— system / info / backup。
    // 它由 POST /api/admin/backup/remote/restore 触发，属 HTTP 调用，所以带请求元数据。
    // （原来的示例把这里的级别写成 warn，真实是 info）
    ...
    metadata: JSON.stringify({
      ...
      remotePath: 'nodewarden/backups/nodewarden_backup_2026-09-06.zip',
      method: 'POST',
      path: '/api/admin/backup/remote/restore',
      ip: '203.0.113.42',
      userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/125.0.0.0 Safari/537.36',
    }),
  ...
  {
    // 真实来源：backup.ts 导出备份 —— system / info / backup。
    // 由 POST /api/admin/backup/export 触发，真实调用带了 request，所以带请求元数据。
  ...
  {
    // 真实来源：admin.ts 更新日志中心保留设置 —— system / info / **auditLog**
    id: 'demo-log-system-settings',
    actorUserId: DEMO_USER_ID,
    actorEmail: DEMO_PROFILE.email,
    action: 'admin.audit.settings.update',
    category: 'system',
    level: 'info',
    targetType: 'auditLog',
    targetId: null,
  ...
  {
    // 真实来源：admin.ts 创建邀请 —— system / info / invite
  ...
  {
    // 真实来源：backup.ts 更新备份设置 —— system / info / backup。
    // 由 PUT /api/admin/backup/settings 触发，真实调用带了 request，所以带请求元数据。
